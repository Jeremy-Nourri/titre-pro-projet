...
public class ProjectServiceImpl implements ProjectService {
    ...
    @Override
    public Project updateProject(Long projectId, ProjectDtoRequest request, UserDetails currentUser) {
        User creator = userRepository.findByEmail(currentUser.getUsername())
                .orElseThrow(() -> new RuntimeException("Utilisateur non trouvé"));
        ...
        if (!existingProject.getCreatedBy().getId().equals(creator.getId())) {
            throw new UnauthorizedProjectAccessException("Vous n'avez pas le droit de modifier ce projet.");
        }
    ...
    @Override
    public boolean deleteProject(Long projectId, UserDetails currentUser) {
        User creator = userRepository.findByEmail(currentUser.getUsername())
                .orElseThrow(() -> new RuntimeException("Utilisateur non trouvé"));
        ...
        if (!existingProject.getCreatedBy().getId().equals(creator.getId())) {
            throw new UnauthorizedProjectAccessException("Vous n'avez pas le droit de supprimer ce projet.");
        }
